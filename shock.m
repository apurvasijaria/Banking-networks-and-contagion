...
function [ repaymentAll, Nsurvive] = shockNetwork( Lij, S, R, r, f, fLambda, leverage, endCondition)

%------------------------------------------------------------------------%
%   lending, borrowing, assets, liquid assets, senior liability, and
%   liability are defined according to criticaldegree2.pdf, given fixed
%   input parameters f, flambda, and leverage
%------------------------------------------------------------------------%
lending = sum(Lij,2);
borrowing = sum(Lij)';
assets = lending/f;
...
X1 = repayFrac;
repayment = actualROI + liquidAssets - seniorliability + r*Lij*repayFrac;
...
%------------------------------------------------------------------------%
netWorth = actualROI + liquidAssets - seniorliability + Lij*r*repayFrac - borrowing*r.*repayFrac;
